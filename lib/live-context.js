...
async function loadLiveContext(fetchImpl = fetch, now = new Date()) {
  ...
  const upcomingPublicHolidays = publicHolidays.filter((holiday) => holiday.date >= local.date);
  return {
    ...
    public_holiday_today: holidayToday, upcoming_irish_public_holidays: upcomingPublicHolidays, weather_now: validWeather(weatherData?.current)
  };
}
