...
async function loadServices(fetchImpl = fetch) {
  const cacheIsFresh = servicesCache && Date.now() - servicesCachedAt < CACHE_TTL_MS;
  if (cacheIsFresh) return servicesCache;
  try {
...
function developerPrompt(services) {
...
module.exports = { CACHE_TTL_MS, RESPONSE_FORMAT, SHEET_CSV_URL, conversationText, developerPrompt, loadServices, normaliseHistory, parseCsv, sourcesForIds };
